const HREditor = (() => {
  ...
  const defaultConfig = {
    ...
    translations: {
      ...
      enterNewWidth: "Enter new width (px or %)",
  ...
  const toolbarButtons = {
    ...
    insertImage: (translations) => `<div class="dropdown">
                     ...
                     <div class="dropdown-content">
                         <div class="content-inner">
                             <button type="button" id="imageUploadButton">${translations.uploadImage}</button>
                             <button type="button" id="imageUrlButton">${translations.insertImageURL}</button>
    ...
    insertVideo: (translations) => `<div class="dropdown">
                     ...
                     <div class="dropdown-content">
                         <div class="content-inner">
                             <button type="button" id="videoUploadButton">${translations.uploadVideo}</button>
                             <button type="button" id="videoUrlButton">${translations.insertVideoURL}</button>
  ...
  function createToolbar(config) {
    ...
    (config.toolbar || defaultConfig.toolbar).forEach((button) => {
      toolbar.innerHTML += toolbarButtons[button]
        ? toolbarButtons[button](translations)
        : "";
    });
    return toolbar;
  }
  ...
  function handleToolbarButtonClick(button, translations) {
    ...
    if (commandsWithPrompts[command]) {
      const promptMessage =
        translations[commandsWithPrompts[command]] || "Enter the value";
  ...
  function init(container, config = {}) {
    ...
    const translations = config.translations || defaultConfig.translations;
    console.log(toolbar.querySelectorAll(".heading"));
    ...
    toolbar.querySelectorAll("button[data-command]").forEach((button) => {
      button.addEventListener("click", () =>
        handleToolbarButtonClick(button, translations)
      );
    });
      ...
      if (handler) {
        ...
        reader.onload = (e) => {
          const media = document.createElement(
            event.target.accept.startsWith("image") ? "img" : "video"
          );
          media.src = e.target.result;
          media.className = `hr-editor-${
            event.target.accept.startsWith("image") ? "image" : "video"
          }`;
    ...
    const handleUrlInsert = (type, translations) => {
      const url = prompt(
        translations[`enter${type.charAt(0).toUpperCase() + type.slice(1)}URL`]
      );
      if (url) {
    ...
    toolbar
      ...
      .addEventListener("click", () => imageUploadInput.click());
    imageUploadInput.addEventListener("change", (event) =>
      handleFileUpload(event, imageHandler)
    );

    toolbar
      .querySelector("#imageUrlButton")
      .addEventListener("click", () => handleUrlInsert("img", translations));

    const videoUploadInput = toolbar.querySelector("#videoUpload");
    toolbar
      .querySelector("#videoUploadButton")
      .addEventListener("click", () => videoUploadInput.click());
    videoUploadInput.addEventListener("change", (event) =>
      handleFileUpload(event, videoHandler)
    );

    toolbar
      .querySelector("#videoUrlButton")
      .addEventListener("click", () => handleUrlInsert("video", translations));
    ...
    function addMarginToElement(element, direction) {
      ...
      const newMargin = prompt(
        `${translations.enterMargin} ${direction} (px or %)`,
        style[currentMargin]
      );
      if (newMargin) {
      ...
      button.addEventListener("click", (event) => {
        ...
        if (id.includes("float")) {
          ...
          const newSize = prompt(
            translations.enterNewWidth,
            target.width || target.style.width
          );
          if (newSize) {
